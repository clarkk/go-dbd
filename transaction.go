...
func (t *Tx) Query_row(query sqlc.SQL, scan []any) (bool, error){
	...
	if err != nil {
		return true, &Error{"DB transaction query row compile: "+err.Error(), errors.Wrap(err, 0).ErrorStack()}
	}
	...
	if err := t.tx.QueryRowContext(t.ctx, sql, query.Data()...).Scan(scan...); err != nil {
		...
		if ctx_canceled(err) {
			return true, &Timeout_error{msg, stack}
		}
		return true, &Error{msg, stack}
	}
	return false, nil
}
