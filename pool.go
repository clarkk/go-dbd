...
func Query_row(ctx context.Context, query sqlc.SQL, scan []any) error {
	...
	if err != nil {
		return &Error{"DB query row compile: "+sqlc.SQL_debug(query)+err.Error(), errors.Wrap(err, 0).ErrorStack()}
	}
	
	if err := db.QueryRowContext(ctx, sql, query.Data()...).Scan(scan...); err != nil {
		msg := "DB query row: "+sqlc.SQL_debug(query)+err.Error()
...
func Insert(ctx context.Context, query sqlc.SQL) (int, error){
	...
	sql, err := query.Compile()
	if err != nil {
		return id, &Error{"DB insert compile: "+sqlc.SQL_debug(query)+err.Error(), errors.Wrap(err, 0).ErrorStack()}
	}
	
	if err := db.QueryRowContext(ctx, sql+" RETURNING id", query.Data()...).Scan(&id); err != nil {
		msg := "DB insert: "+sqlc.SQL_debug(query)+err.Error()
		stack := errors.Wrap(err, 0).ErrorStack()
		if ctx_canceled(err) {
